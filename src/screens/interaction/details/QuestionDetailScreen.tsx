...
import { MOCK_QUESTIONS, MOCK_ANSWERS, Answer } from '../mockData';
...
const QuestionDetailScreen = ({ route, navigation }: any) => {
  ...
  const [answers, setAnswers] = useState<Answer[]>(
    MOCK_ANSWERS.filter(a => a.questionId === questionId)
  );
  ...
  if (!question) {
    return (
      <View style={[styles.container, { backgroundColor: colors.background, justifyContent: 'center', alignItems: 'center' }]}>
        <Text style={{ color: colors.text }}>سوال یافت نشد.</Text>
      </View>
    );
  }
  ...
  const handleSend = () => {
    ...
    const newAnswer: Answer = {
      ...
      createdAt: 'همین الان',
      body: replyTo ? `در پاسخ به ${replyTo}:\n${inputText}` : inputText,
      likes: 0
    };
  ...
  const renderHeader = () => (
    <View style={styles.headerComponent}>
      <QuestionCard question={question} onPress={() => {}} />
      <Text style={[styles.sectionTitle, { color: colors.text }]}>پاسخ‌ها</Text>
    </View>
  );
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      <View style={[styles.header, { backgroundColor: colors.surface, borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.text }]}>مباحثه سوال</Text>
        <View style={{ width: 28 }} />
      </View>
      ...
      >
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={[styles.emptyText, { color: colors.textLight }]}>هنوز پاسخی ثبت نشده است.</Text>
          }
        />

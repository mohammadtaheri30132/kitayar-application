...
const ManageClassesScreen = ({ navigation }: any) => {
  ...
  const fetchClasses = async () => {
    try {
      const res = await PlanningService.getClasses();
    ...
    >
      ...
      <View style={styles.detailRow}>
        ...
        <Text style={[styles.detailText, { color: colors.textLight }]}>
          {item.course?.name || item.subjectName || 'درس نامشخص'} {item.grade?.name || item.gradeName ? `· ${item.grade?.name || item.gradeName}` : ''}
        </Text>
      </View>
    </TouchableOpacity>
  );
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      {loading ? (
        ...
        <Text style={[styles.emptyText, { color: colors.textLight }]}>هیچ کلاسی ثبت نشده است.</Text>
      ) : (
        <FlatList
          data={classes}
...
const styles = StyleSheet.create({
  ...
  className: { fontSize: 16, fontWeight: 'bold', fontFamily: 'IRANSansX', marginBottom: 8 },

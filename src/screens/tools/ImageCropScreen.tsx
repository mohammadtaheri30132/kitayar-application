...
import {
  NativeModules, View, Text, TouchableOpacity, FlatList, StyleSheet, Alert, Image
} from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import ReactNativeBlobUtil from 'react-native-blob-util';
import { Share2, Eye, Trash2, Download, Crop } from 'lucide-react-native';
...
const { FileShareModule, FileSaverModule, ImageCropModule } = NativeModules;
const TOOL_ID = 'IMAGE_CROPPER';

const ImageCropScreen = ({ navigation }) => {
  ...
  const handleStartCrop = async () => {
    try {
      ...
      const resultPath = await ImageCropModule.openCropMaker(uri);
      if (resultPath) {
      ...
      if (err !== 'CANCEL' && err !== 'CANCELLED' && err?.code !== 'CANCELLED') {
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.actionBox}>
        <View style={styles.actionHeader}>
          <Crop color={COLORS.primary} size={35} />
          <Text style={styles.actionTitle}>برش تصویر (Crop)</Text>
        </View>
        <Text style={styles.actionDesc}>عکس خود را انتخاب کنید و بخش دلخواه آن را ببرید.</Text>
        <TouchableOpacity style={styles.convertButton} onPress={handleStartCrop}>
          <Text style={styles.convertButtonText}>انتخاب عکس و برش</Text>
        </TouchableOpacity>
      </View>

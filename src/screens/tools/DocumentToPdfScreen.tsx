...
import { Share2, Trash2, Download, FileText, FileSpreadsheet, Presentation, File, Eye } from 'lucide-react-native';
...
const { FileShareModule, FileSaverModule, TextToPdfModule } = NativeModules;

// تنظیمات پویا بر اساس نوع ابزار
const TOOL_CONFIG = {
  TEXT: {
    id: 'TEXT_TO_PDF',
    title: 'تبدیل متن به PDF',
    desc: 'فایل متنی (.txt) خود را انتخاب کنید تا به صورت آفلاین به PDF تبدیل شود.',
    mimeType: 'text/plain',
    Icon: FileText,
    iconColor: '#34495E'
  },
  WORD: {
    id: 'WORD_TO_PDF',
    title: 'تبدیل ورد به PDF',
    desc: 'فایل Word خود را انتخاب کنید تا از طریق سرور به PDF تبدیل شود.',
  ...
  EXCEL: {
    id: 'EXCEL_TO_PDF',
    title: 'تبدیل اکسل به PDF',
    desc: 'فایل Excel خود را انتخاب کنید تا از طریق سرور به PDF تبدیل شود.',
  ...
  POWERPOINT: {
    id: 'PPT_TO_PDF',
    title: 'تبدیل پاورپوینت به PDF',
    desc: 'فایل PowerPoint خود را انتخاب کنید تا از طریق سرور به PDF تبدیل شود.',
...
const DocumentToPdfScreen = ({ route, navigation }) => {
  // دریافت نوع ابزار از ناوبری (مثلا 'TEXT' یا 'WORD')
  const docType = route.params?.type || 'TEXT';
  const config = TOOL_CONFIG[docType];

  const [history, setHistory] = useState([]);
  const [savedItems, setSavedItems] = useState({});
  const [isLoading, setIsLoading] = useState(false);

  useEffect(() => { loadHistory(); }, [docType]);
  const loadHistory = async () => { setHistory(await getToolHistory(config.id)); };

  const handlePickAndConvert = async () => {
    try {
      // استفاده از پیکر جدید با فیلتر پسوند
      const urisArray = await FilePickerModule.pickDocument(config.mimeType);
      ...
      if (docType === 'TEXT') {
        // پردازش کاملا بومی و آفلاین برای متن
        resultPath = await TextToPdfModule.convertTextToPdf(uri);
      } else {
        // شبیه‌سازی ارسال به بک‌اند Node.js برای فایل‌های آفیس
        // در اینجا باید متد fetch یا axios خود را برای ارسال فایل به API قرار دهید
        await new Promise(resolve => setTimeout(resolve, 2500)); 
        Alert.alert("توجه", "در این بخش فایل باید به API بک‌اند شما ارسال شود تا فرمت و جداول حفظ شوند.");
        setIsLoading(false);
        return;
      }
      ...
      await saveToolHistory(config.id, fileName, resultPath, sizeStr, 'PDF');
      loadHistory();
      Alert.alert("عملیات موفق", "فایل شما با موفقیت به PDF تبدیل شد.");
    } catch (err) {
      if (err !== 'CANCELLED' && err?.code !== 'CANCELLED') {
        Alert.alert("خطا", err.message || "خطا در پردازش فایل");
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDelete = async (id) => { setHistory(await deleteToolHistory(config.id, id)); };
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.actionBox}>
        <View style={styles.actionHeader}>
          <IconComponent color={config.iconColor} size={35} />
          <Text style={styles.actionTitle}>{config.title}</Text>
        </View>
        <Text style={styles.actionDesc}>{config.desc}</Text>
        
        <TouchableOpacity 
          style={[styles.convertButton, { backgroundColor: config.iconColor }]} 
          onPress={handlePickAndConvert} 
          disabled={isLoading}
        >
          {isLoading ? <ActivityIndicator color="#fff" /> : <Text style={styles.convertButtonText}>انتخاب فایل و تبدیل</Text>}
        </TouchableOpacity>
      </View>
          ...
          <View style={styles.historyCard}>
            ...
            <View style={styles.historyContent}>
              <View style={[styles.iconWrapper, { backgroundColor: config.iconColor + '15' }]}>
                <FileText color={config.iconColor} size={30} />
              </View>
            ...
            <View style={styles.actionButtonsRow}>
              ...
              <TouchableOpacity style={[styles.actionBtn, styles.viewBtn]} onPress={() => navigation.navigate('PdfViewerScreen', { path: `file://${item.resultPath}` })}>
...
const styles = StyleSheet.create({
  ...
  actionDesc: { fontSize: 13, color: '#7f8c8d', textAlign: 'center', marginBottom: 20, lineHeight: 22 },
...
export default DocumentToPdfScreen;
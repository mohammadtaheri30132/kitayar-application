...
const DateDifferenceScreen = ({ navigation }: any) => {
  ...
  const handleCalculate = () => {
    ...
    try {
      // The function auto-swaps if end is before start. 
      // If we want to show a validation instead, we could check isBefore(endDate, startDate)
      const diff = getCalendarDifference(startDate, endDate);
      setResult(diff);
    } catch (e) {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
      <View style={[styles.header, { backgroundColor: colors.surface, borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.text }]}>فاصله بین دو تاریخ</Text>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            ...
            >
              <Text style={styles.calcButtonText}>محاسبه فاصله</Text>
            </TouchableOpacity>
          </View>
          ...
          {result && (
            <DateResultCard
              ...
              secondaryValue={`تعداد کل روزها: ${result.totalDays.toLocaleString('fa-IR')} روز`}
            />
          )}
